...
const InstituteGallery: React.FC = () => {
    ...
    // Group images in sets of 4 for each slide
    const chunkArray = (arr: GalleryImage[], size: number) =>
        arr.reduce<GalleryImage[][]>((acc, _, i) => {
            if (i % size === 0) acc.push(arr.slice(i, i + size));
            return acc;
        }, []);

    const groupedSlides = chunkArray(galleryImages, 4);
